...
def gp_dist_full_log_prob(dm_parameter, dists, l, v, nugget, kernel_type, dists_dtype):
    '''
    Compute the log probability of a parameter vector under a GP prior
    * Full covariance matrix method

    - dm_parameter: [n_vx] demeaned parameter vector
    - dists: [n_vx, n_vx] distance matrix
    - l: lengthscale
    - v: variance
    - nugget: nugget term for numerical stability
    - kernel_type: 'RBF', 'matern52', 'laplace'
    '''
    dists_dtype = tf.float64
    if kernel_type == 'RBF':
        cov_matrix = tf.square(v) * tf.exp(
            -tf.square(dists) / (2.0 * tf.square(l))
        )
    elif kernel_type == 'matern52':
        sqrt5 = tf.cast(tf.sqrt(5.0), dtype=dists_dtype)
        frac1 = (sqrt5 * dists) / l
        frac2 = (5.0 * tf.square(dists)) / (3.0 * tf.square(l))
        cov_matrix = tf.square(v) * (1 + frac1 + frac2) * tf.exp(-frac1)
    elif kernel_type == 'laplace':
        cov_matrix = tf.square(v) * tf.exp(-dists / l)
    else:
        raise ValueError("Unsupported kernel: {}".format(kernel_type))
    cov_matrix += tf.eye(cov_matrix.shape[0], dtype=dists_dtype) * tf.cast(nugget, dtype=dists_dtype)
