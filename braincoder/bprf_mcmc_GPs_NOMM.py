...
from braincoder.bprf_mcmc_GPs import GPdistsM

class GPSpecNoMM(GPdistsM):
    ...
    def _return_kappa_diag(self, inducing_indices, **kwargs):
        ...
        sigma_phi = tf.matmul(s_out, eig_vects)  # [N, L]
        k_out += tf.reduce_sum(eig_vects * sigma_phi, axis=0)  # [L]        
    ...
    def _return_kappa_dense(self, inducing_indices, **kwargs):
        ...
        s_out = self._return_sigma_component(inducing_indices=inducing_indices, **kwargs)
        k_out += sigma2kappa_dense(s_out, eig_vects)        
    ...
    def _return_log_prob_unfixed(self, parameter, n_inducers=None, **kwargs):
        ...
        y_recon = tf.tensordot(eig_vects, proj_dm_parameter, axes=[[1],[0]])  # [N]
...
# **************************** 
@tf.function
def sigma2kappa_diag(sigma, eig_vects):
    """
    ...
    sigma_phiw = tf.matmul(sigma, eig_vects)            # [N, L]

    # extract diagonal entries:
    kappa = tf.reduce_sum(sigma_phiw, axis=0)           # [L]

    return kappa

@tf.function
def sigma2kappa_dense(sigma, eig_vects):
    """
    Project a dense covariance matrix into the spectral domain,
    ...
    """
    sigma_mp = tf.matmul(sigma, eig_vects)                         # [N, L]
    kappa = tf.matmul(eig_vects, sigma_mp, transpose_a=True)       # [L, L]
...
def kappa_diag2sigma(kappa, eig_vects):
    ...
    sigma_recon = tf.matmul(eig_vects, tf.matmul(Lambda, eig_vects, transpose_b=True))  # [N, N]
    # ensure symmetry
...
def kappa_dense2sigma(kappa, eig_vects):
    ...
    Returns:
        sigma_recon: tf.Tensor of shape (N, N), reconstructed Σ.
    """
    sigma_recon = tf.matmul(eig_vects, tf.matmul(kappa, eig_vects, transpose_a=True))  # [N, N]
    return 0.5 * (sigma_recon + tf.transpose(sigma_recon))
